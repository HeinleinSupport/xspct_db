...
class RspamdSettingsResponse(BaseModel):
    ...
    actions: dict[str, float | str] = Field(..., description="Score thresholds (reject, greylist, add header, …).")
    ...
    symbols: list[str] | dict[str, float] = Field(
        default_factory=list,
        description="Rspamd symbols to force (list) or symbol→score pairs (dict).",
    )
    subject: str | None = Field(default=None, description="Rspamd subject rewrite template.")
